...
static std::vector<std::string> glaph_nodes; 
...
static R3Point initial_node_cood(0,0,0);
static RNBoolean initial_node = FALSE;
...
ChangeCol(int num)
{
  float col_w = 0.05 * num;
  float red = std::max({1.0 - col_w, 0.0});
  float green = col_w;
  if(green > 1.0) green = 2.0 - col_w;
  if(green < 0.0) green = 0.0;
  float blue = std::max({-1.0 + col_w, 0.0});
  std::array<float, 3> col = {red, green, blue};
  return col;
}

// static void
// DrawAxes(std::vector<std::string> nodes)
// {
//   // Draw axes
//   R3Point previous_node(0.0, 0.0, 0.0);
...
// Output: Path
static void
DrawPath(std::vector<std::string> nodes)
{
  R3Point previous_node(0.0, 0.0, 0.0);
  RNArray<MPPanorama *> tmp_panorama = house->panoramas;
  // 受け取ったリストの長さを格納
  int num_node = nodes.size();
  int glaph_ele = 0;
  for(int i = 0; i < num_node; i++){
    try{
      std::string tmp_nodes = nodes.at(i);
    } catch(const std::out_of_range& oor){
      std::cout << "ERROR: " << oor.what() << std::endl;
    }

    // ノードの座標を獲得
    std::array<float, 3> node_pos = {0.0, 0.0, 0.0};
    for(int j = 0; j < house->panoramas.NEntries(); j++){
      if(tmp_panorama[j]->name == nodes[i]){
        node_pos[0] = tmp_panorama[j]->position[0];
        node_pos[1] = tmp_panorama[j]->position[1];
        node_pos[2] = tmp_panorama[j]->position[2];
        break;
      }
    }
    R3Point tmp_point(node_pos[0], node_pos[1], node_pos[2]);

    // ノードの描画
    glLineWidth(1000);
    std::array<float, 3> col = ChangeCol(glaph_ele);
    glaph_ele++;
    R3BeginLine();
    glColor3f(col[0], col[1], col[2]);
    R3LoadPoint(tmp_point + 0.05 * R3negz_vector);
    R3LoadPoint(tmp_point + 0.05 * R3posz_vector);
    R3EndLine();
    R3BeginLine();
    glColor3f(col[0], col[1], col[2]);
    R3LoadPoint(tmp_point + 0.05 * R3negx_vector);
    R3LoadPoint(tmp_point + 0.05 * R3posx_vector);
    R3EndLine();
    R3BeginLine();
    glColor3f(col[0], col[1], col[2]);
    R3LoadPoint(tmp_point + 0.05 * R3negy_vector);
    R3LoadPoint(tmp_point + 0.05 * R3posy_vector);
    R3EndLine();
    glLineWidth(3);
    ...
    if(i >= 1){
      ...
      R3BeginLine();
      glColor3f(col[0], col[1], col[2]);
      R3LoadPoint(previous_node);
      R3LoadPoint(tmp_point);
      R3EndLine();
    }
    previous_node = {node_pos[0], node_pos[1], node_pos[2]};
...
void GLUTRedraw(void)
{
  ...
  //   glDisable(GL_LIGHTING);
  //   glLineWidth(3);
  //   DrawAxes(glaph_nodes);
  //   glLineWidth(1);
  // }
  std::ifstream nodes_f(path_filename);
  std::string nodes;
  while(std::getline(nodes_f, nodes)){
      std::string node;
      std::istringstream stream(nodes);
      while (std::getline(stream, node, ' ')){
        glaph_nodes.push_back(node);
      }
  }
  glDisable(GL_LIGHTING);
  glLineWidth(3);
  DrawPath(glaph_nodes);
  glLineWidth(1);
...
ParseArgs(int argc, char **argv)
{
  ...
  while (argc > 0) {
    if ((*argv)[0] == '-') {
      if (!strcmp(*argv, "-v")) print_verbose = 1;
      ...
      else if (!strcmp(*argv, "-path")) { argc--; argv++; path_filename = *argv; }
        ...
        argv++; argc--; y = atof(*argv);
        argv++; argc--; z = atof(*argv);
        initial_node_cood = R3Point(x, y, z);
        initial_node = TRUE;
      }
      else {
